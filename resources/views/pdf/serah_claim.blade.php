...
<body>
    ...
    <table class="meta-table">
        <tr><td class="meta-label">Nomor</td><td class="meta-colon">:</td><td><strong>{{ $claim->nomor_claim }}</strong></td></tr>
        <tr><td class="meta-label">Nama</td><td class="meta-colon">:</td><td>{{ $claim->user->name }}</td></tr>
        <tr><td class="meta-label">Divisi</td><td class="meta-colon">:</td><td>{{ $claim->user->division->nama_divisi ?? '-' }}</td></tr>
    ...
    <table class="items-table">
        ...
        <tbody>
            @foreach($claim->details as $i => $detail)
                @php
                    $baseDetail = $detail->requestDetail;
                    $info = $detailStockInfo[$detail->id] ?? null;
                    $hargaSatuan = (int) ($info['harga_satuan'] ?? ($baseDetail->harga_manual ?? ($baseDetail->barang->harga_estimasi ?? 0)));
                    $estimasi = (int) ($info['estimasi'] ?? (((int) $detail->qty) * $hargaSatuan));
                    $totalEstimasi += $estimasi;
                @endphp
                <tr>
                    <td class="text-center">{{ $i + 1 }}</td>
                    <td>{{ $baseDetail->barang->nama_barang ?? '-' }}</td>
                    <td>{{ $baseDetail->keterangan ?: '-' }}</td>
